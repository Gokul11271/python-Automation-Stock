...
# =========================================================
# FIXED STATIC SEQUENCE HELPERS
# =========================================================
current_odd_lot = 0.01   
current_even_lot = 0.02  

def get_next_odd(lot):
    return round(lot + 0.02, 2)

def get_next_even(lot):
    return round(lot + 0.02, 2)
...
# =========================================================
# TARGETED CLEANUP PENDINGS (CRITICAL UPDATE)
# =========================================================
def cancel_all_pending():
    """Hard clear for profit target shutdowns."""
    orders = my_orders()
    for o in orders:
        mt5.order_send({"action": mt5.TRADE_ACTION_REMOVE, "order": o.ticket})

def cancel_outer_pending(low_lvl, high_lvl):
    """Flushes out ONLY outer boundary orders during even triggers, leaving inner odd pendings untouched."""
    orders = my_orders()
    for o in orders:
        # If the pending order is sitting on the outer boundaries, remove it
        if abs(o.price_open - low_lvl) < 0.01 or abs(o.price_open - high_lvl) < 0.01:
            mt5.order_send({
...
# INITIALIZE STEP 1
# =========================================================
log(f"🚀 Initial BUY lot={current_odd_lot} triggered at Inner Level")

buy_result = market_buy(current_odd_lot)
...
# Set the Initial Step 1 Traps
place_pending("SELL", GRID_LOW, current_even_lot)          
place_pending("SELL", GRID_MID_HIGH, current_odd_lot)     
...
while True:
    ...
    if latest_pos and latest_pos.ticket not in known_tickets:
        ...
        is_odd_volume = round(pos.volume * 100) % 2 != 0

        # -----------------------------------------------------------
        # CONDITION 1: ODD VOLUME TRIGGERED (RETAIN MATRIX, OVERLAY EVEN)
        # -----------------------------------------------------------
        if is_odd_volume:
            log(f"ℹ️ Odd volume hit ({pos.volume}). Retaining layers. Preparing Even protection.")
            
            if pos.type == mt5.POSITION_TYPE_SELL: 
                place_pending("BUY", GRID_HIGH, current_even_lot)
            else: 
                place_pending("SELL", GRID_LOW, current_even_lot)

        # -----------------------------------------------------------
        # CONDITION 2: EVEN VOLUME TRIGGERED (FLUSH ONLY OUTER, ADVANCE ODD)
        # -----------------------------------------------------------
        else:
            log(f"🚨 Even volume hit ({pos.volume}). Flushing ONLY Outer matrix channels.")
            
            # Core Adjustment: Protect inner odds by only clearing boundary levels
            cancel_outer_pending(GRID_LOW, GRID_HIGH) 
            
            current_odd_lot = get_next_odd(current_odd_lot)
            current_even_lot = get_next_even(pos.volume)
            
            log(f"🔄 Sequence Advanced: Next Inner Slots={current_odd_lot} | Next Outer Slots={current_even_lot}")

            if pos.type == mt5.POSITION_TYPE_SELL: 
                place_pending("BUY", GRID_MID_LOW, current_odd_lot)
                place_pending("SELL", GRID_MID_HIGH, current_odd_lot)
            else: 
                place_pending("SELL", GRID_MID_HIGH, current_odd_lot)
                place_pending("BUY", GRID_MID_LOW, current_odd_lot)
    ...
    ax.set_title(f"Profit={current_profit:.2f}", fontsize=8)
    ax.grid(True)
    plt.tight_layout()
    plt.pause(0.01)
