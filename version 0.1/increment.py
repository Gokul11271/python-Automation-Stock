...
def volume_pattern():
    ...
    n = 1
    while True:
        yield n * 0.01   # convert sequence into broker-compatible lots
...
PROFIT_TARGET = float(input("Enter profit target in $: "))

# Get first volume
current_volume = next(vol_gen)

print(f"🚀 Starting cycle with BUY STOP at {buy_price}, gap = {gap}, profit target = {PROFIT_TARGET}$")
...
try:
    ...
    while True:
        ...
        if positions:
            ...
            # ✅ check profit AFTER each order
            account = mt5.account_info()
            if account and account.profit >= PROFIT_TARGET:
