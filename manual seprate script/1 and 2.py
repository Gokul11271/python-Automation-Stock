...
def volume_pattern_generator():
    yield 0.01        # first order volume
    yield 0.02        # second order volume
    while True:
        yield 0.02    # all subsequent orders
...
def run_cycle(vol_gen, gap):
    ...
    active_price = place_pending_stop("BUY", buy_price, next(vol_gen))
    if not active_price:
        return "error"
    ...
    while True:
        ...
        if current_count > last_pos_count:
            new_positions = [p for p in positions if p.ticket not in [lp.ticket for lp in last_positions]]
            for pos in new_positions:
                triggered_count += 1
                vol = pos.volume
                incremental_tp = vol * PROFIT_UNIT
                cumulative_tp += incremental_tp

                order_log.append({
                    "ticket": pos.ticket,
                    "type": "BUY" if pos.type == mt5.POSITION_TYPE_BUY else "SELL",
                    "volume": vol,
                    "cumulative_tp": round(cumulative_tp, 2)
                })

                printl(f"\n🔔 Trigger #{triggered_count} → ticket={pos.ticket}, vol={vol}, cumulative TP=${cumulative_tp}")

                # Place the next STOP order based on gap
